...
/**
 * Servlet implementation class MQChartJSONServlet
 */
public class MQChartJSONServlet extends HttpServlet {
	...
	private void processRequest(HttpServletRequest request,
		...
		try{
			...
			Collection<MQQuequeReport> collection=db.getMqByMonthSystem(year,month, system);
			int i=0;
			for(MQQuequeReport el:collection){
				if(i==0){
					...
					categories=categories.concat("\""+el.getDate()+","+el.getDayOfWeek()+"\"");
					i++;
				}
				else{
					putSeries=putSeries.concat(", "+String.valueOf(el.getPutBytes()));
